...
func TestRunObfuscateRejectsUnknownFlags(t *testing.T) {
	args := []string{"--input", "/tmp/in", "--output", "/tmp/out", "--bogus-flag"}
	if got := RunObfuscate(args); got != 1 {
		t.Fatalf("expected exit 1 for unknown flag, got %d", got)
	}
}
