#!/usr/bin/env bash
#
# E2E coverage lifecycle script for CI and local use (must-gather-operator).
#
# Usage:
...
NAMESPACE="${NAMESPACE:-must-gather-operator}"
DEPLOYMENT="${DEPLOYMENT:-must-gather-operator}"
OPERATOR_CONTAINER="${OPERATOR_CONTAINER:-must-gather-operator}"
...
setup() {
    ...
    if [[ -z "${csv}" ]]; then
        echo "Error: no CSV found for deployment ${DEPLOYMENT} in ${NAMESPACE}"
        exit 1
    fi
    echo "Found CSV: ${csv}"
    ...
    oc exec -n "${NAMESPACE}" "deploy/${DEPLOYMENT}" -c "${OPERATOR_CONTAINER}" -- env | grep GOCOVERDIR || \
...
collect() {
    ...
    oc exec -n "${NAMESPACE}" "${pod}" -c "${OPERATOR_CONTAINER}" -- /bin/sh -c 'kill -TERM 1' || true
    ...
    oc cp "${NAMESPACE}/${pod}:${GOCOVERDIR_PATH}/." "${coverage_dir}" -c "${OPERATOR_CONTAINER}"
