	...
	tests := []struct {
		...
		},
		{
			...
			interceptors: func() interceptClient {
				return interceptClient{
					onGet: func(ctx context.Context, key client.ObjectKey, obj client.Object) error {
						// Return not found for job to trigger creation path
						if _, ok := obj.(*batchv1.Job); ok && key.Name == "example-mustgather" {
							return apierrors.NewNotFound(batchv1.Resource("job"), key.Name)
						}
						// Return not found for secret in operator namespace to trigger creation
						if _, ok := obj.(*corev1.Secret); ok && key.Namespace == defaultMustGatherNamespace && key.Name == "secret" {
							return apierrors.NewNotFound(corev1.Resource("secret"), key.Name)
						}
						return nil
					},
				}
		...
		},
		{
			...
			interceptors: func() interceptClient {
				return interceptClient{
					onGet: func(ctx context.Context, key client.ObjectKey, obj client.Object) error {
						// Return not found for the secret in operator namespace to trigger create
						if _, ok := obj.(*corev1.Secret); ok && key.Namespace == defaultMustGatherNamespace && key.Name == "secret" {
							return apierrors.NewNotFound(corev1.Resource("secret"), key.Name)
						}
						return nil
